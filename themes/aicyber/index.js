...
// 主题组件
const AICyberTheme = ({ children, layout, fullWidth }) => {
  // 添加赛博朋克特效
  useEffect(() => {
    // 添加字体
    const link = document.createElement('link')
    link.href = 'https://fonts.googleapis.com/css2?family=Orbitron:wght@400;500;700&family=Rajdhani:wght@300;400;500;700&family=Fira+Code&display=swap'
    link.rel = 'stylesheet'
    document.head.appendChild(link)
    
    // 添加故障效果
    if (CONFIG.CYBERPUNK_FEATURES.ENABLE_GLITCH_EFFECT) {
      const glitchEffect = () => {
        const headings = document.querySelectorAll('h1, h2')
        headings.forEach(heading => {
          if (Math.random() < 0.05) { // 5%的概率触发故障效果
  ...
  return (
    <div className={`aicyber-theme ${fullWidth ? 'full-width' : ''}`}>
      <Style />
      {/* 现有布局代码... */}
      {children}
    </div>
...
// 导出组件
export default AICyberTheme
export { CyberGlitchText, CyberButton, CyberCard }
